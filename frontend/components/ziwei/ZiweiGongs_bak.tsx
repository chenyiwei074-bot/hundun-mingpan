      ...
      <div className="px-4 sm:px-6 pb-6">
        <div className="grid grid-cols-4 max-w-lg mx-auto" style={{gap:"1px",background:colors.hairline}}>
          {[1,2,3,4].map(row=>[1,2,3,4].map(col=>{
            // 中心太极区 (row 2-3, col 2-3)
            if(row>=2&&row<=3&&col>=2&&col<=3){
              if(row===2&&col===2) return (
                <div key="center" className="row-span-2 col-span-2 flex items-center justify-center bg-white py-8">
                  <div className="text-center">
                    <div className="w-[88px] h-[88px] sm:w-[104px] sm:h-[104px] mx-auto rounded-full flex items-center justify-center"
                      style={{background:"linear-gradient(135deg, rgba(178,149,93,0.05), transparent)"}}>
                      <span className="font-serif font-bold tracking-[0.3em]" style={{fontSize:t.body.size,color:"rgba(178,149,93,0.35)"}}>太极</span>
                    </div>
                    <p style={{fontSize:t.micro.size,color:"rgba(0,0,0,0.12)"}} className="mt-2 tracking-[0.2em]">紫微斗数</p>
                  </div>
                </div>
              );
              return null;
