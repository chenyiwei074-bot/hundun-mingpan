  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-xuan-zhi flex flex-col items-center justify-center">
        ...
        <p className="text-dai-qing/60 text-sm mt-4 tracking-[2px]">加载命盘...</p>
      </div>
    );
  }
        ...
        <div className="rounded-2xl border border-dai-qing/10 bg-xuan-zhi-dark p-6">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-center text-sm">
              ...
              <tbody>
                ...
                <tr className="border-b border-dai-qing/5 text-xs text-dai-qing/50">
                  {pillars.map(p => {
                    const cg = bz.cangGan?.[p] || [];
                    return <td key={p} className="py-2">{Array.isArray(cg) ? cg.map((g:any) => g?.gan||'').filter(Boolean).join(' ') : '-'}</td>;
                  })}
                </tr>
              </tbody>
            </table>
          ...
          <div className="grid grid-cols-4 gap-3 mt-5 text-center text-xs">
            ...
            <div className="rounded-xl border border-dai-qing/8 bg-dai-qing/3 px-3 py-2">
              ...
              <p className="text-dai-qing text-sm mt-0.5">{bz.geju || '-'}</p>
            </div>
            <div className="rounded-xl border border-dai-qing/8 bg-dai-qing/3 px-3 py-2">
              <p className="text-dai-qing/40">旺衰</p>
              <p className="text-dai-qing text-sm mt-0.5">{bz.wangshuai || '-'}</p>
            </div>
            <div className="rounded-xl border border-dai-qing/8 bg-dai-qing/3 px-3 py-2">
        ...
        <div className="rounded-2xl border border-dai-qing/10 bg-xuan-zhi-dark p-6">
          ...
          {zw.gongs ? (
            <div className="grid grid-cols-4 gap-1.5">
              {(['巳','午','未','申','辰','酉','卯','戌','寅','丑','子','亥'] as string[]).map(pos => {
            ...
            <p className="text-center text-dai-qing/40 text-sm">紫微数据加载中...</p>
          )}
