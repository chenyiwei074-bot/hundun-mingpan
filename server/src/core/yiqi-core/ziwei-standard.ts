...
import { addBenMingSiHua, getLaiYinGong, detectZiHua, getSiHuaOverview } from './sihua';
...
export function createZiweiChart(birthInfo: BirthInfo): ZiweiChart {
  try {
    ...
    // 11b. 许诠仁体系：来因宫 + 自化 + 四化总览
    const laiYinGong = getLaiYinGong(yearGan as any, gongsWithSiHua);
    const gongsWithZiHua = detectZiHua(gongsWithSiHua);
    const sihuaOverview = getSiHuaOverview(yearGan as any, gongsWithZiHua);
    ...
    const gongsWithDaXian = gongsWithZiHua.map((gong, index) => {
      ...
      if (currentDaXian) {
        // 找到当前大限的起始宫位在十二宫中的位置
        const currentDaXianGongIndex = gongsWithZiHua.findIndex(g => {
    ...
    return {
      ...
      laiYinGong: laiYinGong ? {
        gong: laiYinGong.gong as any,
        dizhi: laiYinGong.dizhi as any,
        tiangan: laiYinGong.tiangan as any,
        desc: laiYinGong.desc
      } : undefined,
      sihuaOverview: sihuaOverview ? {
        lu: sihuaOverview.lu,
        quan: sihuaOverview.quan,
        ke: sihuaOverview.ke,
        ji: sihuaOverview.ji,
        summary: sihuaOverview.summary
      } : undefined,
