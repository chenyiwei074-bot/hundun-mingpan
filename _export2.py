...
skip_ext = {".png",".jpg",".webp",".ico",".tar.gz",".db",".tsbuildinfo",".lock",".gz",".zip",".woff2",".svg",".pdf"}
...
with open(out, "w", encoding="utf-8") as of:
    of.write("# HDAI (hundunmp.vip) - Complete Source Code Export\n\n")
    of.write("## Project Structure\n\n")
    of.write("```\n")
    ...
    for rel, sz, fp in files:
        ...
        lang_map = {"tsx":"tsx","ts":"typescript","js":"javascript","py":"python","css":"css","json":"json","yml":"yaml","yaml":"yaml","md":"markdown","sql":"sql","prisma":"prisma","html":"html","env":"bash","txt":"","sh":"bash","toml":"toml","xml":"xml"}
        ...
        try:
            with open(fp, "r", encoding="utf-8") as sf:
                content = sf.read()
            # limit individual files to 5000 lines
            lines = content.split("\n")
            if len(lines) > 5000:
                content = "\n".join(lines[:5000]) + "\n... (truncated)"
            of.write(content)
...
print(f"OK: {len(files)} files, {size_mb:.1f} MB")